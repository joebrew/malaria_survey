...
df <- 
  ...
  mutate(years = make_numeric(years),
         height = make_numeric(height),
         sleep = make_numeric(sleep),
         age = make_numeric(age))

# Throw out any garbage data
df <- df %>%
  filter(age > 18,
         age < 45,
         height > 140,
...
plot_variable <- function(variable = 'sleep',
                          data = df){
...
plot_variable_by <- function(variable = 'sleep',
                             variable_by = '',
                             data = df){
  data <- data.frame(data)
  data$x <- data[,variable]
