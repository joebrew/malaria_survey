...
ols1 = lm(years ~ sentiment  + area_epidemiology + area_anthropology, data = df[df$responded,])
...
# Heckman 2 step
heck1 <- heckit(responded ~ gender + ethnicity,
                years ~ sentiment + area_epidemiology + area_anthropology, data = df)

# Maximum likelihood (like heckman 2 step, but slightly different)
# ML estimation of selection model
ml1 = selection(responded ~ gender + ethnicity,
                years ~ sentiment + area_epidemiology + area_anthropology, data = df)
...
De-biasing of sample selection was carried out using Heckman's 2 step approach (citation needed), as implemented by Toomet [@toomet2008].

# Results

A total of `r nrow(df)` researchers participated in the survey.

## Sample selection and de-biasing of results

This section is unfinished. Below are regression outputs for both OLS and Heckman's 2 step model. I will update this soon.

```{r}
# stargazer(ols1, heck1, ml1,    
#           # se=list(cse(ols1),NULL,NULL), 
#           title="Regression on years until eradication", type="html",
#           df=FALSE, digits=4)
summary(ols1)
summary(heck1)
...
Participants had a total of `r length(old_areas)` unique (self-described) areas of expertise, of which `r length(areas)` areas of expertise had 5 or more participants. 
